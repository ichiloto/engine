...
| G1 | Independent graphical battle layout, static arena and graphical combatants. Engine integration with Renderer; Game Design and Art. | Agreed canvas contract and matching fixtures; native implementation; approved graphical layout/assets. | Accepted and integrated into the normal local Game, including native UI, right-aligned resources, recipient-side feedback and target cursors. [Current contract and validation](graphical-battle-g1.md) is authoritative; superseded temporary runtimes are removed. Latest accepted Engine source passes 2079 tests on PHP 8.4/8.5 with one existing skip each. Game production-binding checks pass 11 tests / 392 assertions; native macOS encounter and corrected Cure placement passed. Broader platform/outcome coverage and future actor-cursor polish are explicitly limited there. Local integration is not remote publication. |
