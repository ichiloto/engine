...
final class TitleMenuPresentation
{
    ...
    bool $showMenu = true): PresentationCanvas
  {
    ...
    foreach (['day' => 1.0, 'night' => $mix] as $name => $opacity) {
      ...
      if (PngAssetPreflight::getAvailableSize($catalog->assetRoot, $scene['background']) === null) { continue; }
      $layer = $name === 'day' ? 0 : 2;
      ...
      if (!$playback->reducedMotion && $effects !== [] && $ambient > 0) {
        ...
        $composites[] = new CanvasComposite('title-' . $name, PresentationCanvas::DEFAULT_WIDTH, PresentationCanvas::DEFAULT_HEIGHT, $bounds, $operations, $layer, $opacity);
    ...
    $ui = $view->finish();
    $ui = new PresentationCanvas(PresentationCanvas::DEFAULT_WIDTH, PresentationCanvas::DEFAULT_HEIGHT,
