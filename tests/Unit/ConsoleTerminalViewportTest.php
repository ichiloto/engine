...
it('clips composed overlays at the physical edge without losing the updated underlay', function (bool $batched) {
  Console::syncDimensions(8, 3);
  Console::syncTerminalViewport(5, 2, repaint: false);
  Console::write('abcdefgh', 0, 0);
  Console::replaceOverlay('notice', ["\u{754C}Z"], 4, 0, 2000);
  ob_clean();
  if ($batched) { Console::beginFrame(); }
  Console::write('XYZ', 4, 0);
  Console::repaintRegion(4, 0, 3, 1);
  if ($batched) { Console::endFrame(); }
  expect(ob_get_contents())->toBe("\e[1;5H ");
  ob_clean();
  Console::removeOverlay('notice');
  expect(ob_get_contents())->toBe("\e[1;5HX")
    ->and(Console::getBuffer()[0])->toBe('abcdXYZh');
  ob_clean();
  Console::syncTerminalViewport(8, 3);
  expect(ob_get_contents())->toContain('abcdXYZh');
})->with([false, true]);
